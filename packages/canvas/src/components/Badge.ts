/**
 * 徽章：需求单的徽章 D（卡角状态角标）、I（敬请期待角标）。
 *
 * 原来还有 E（中线回合徽章）：正式版简化第 4 步之二把战场中线那块匾换成素方块之后
 * 它没有调用方了。A（费用圆章）、B（问号帮助圆章）、C（卡面铭牌）是第 4 步之三去掉的
 * ——那三枚只长在卡面上，而卡上的每一层都要过透视投影、是四边形网格
 *（见 CardSprite 的文件头），普通容器挂上去就没有近大远小了，所以卡牌从来没用过它们，
 * 这三个变体建出来就一直没有调用方。形状仍在 fx/badgeShapes.ts 和 fx/cardShapes.ts，
 * 由卡牌那批预烤纹理直接用。
 *
