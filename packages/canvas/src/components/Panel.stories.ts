 ...
 * 需求单里面板 A / B / D / M 的状态那一行写的都是「普通 ✓ · 其余 —」。
 *
...
function mount(ctx: StoryStage, options: PanelOptions) {
  ...
  const panel = new Panel({ ...options, width: given?.width, height: given?.height })
  panel.position.set((ctx.width - panel.boxWidth) / 2, (ctx.height - panel.boxHeight) / 2)
...
function spec(options: PanelOptions) {
  return {
    pixi: { ...SIZE, mount: (ctx: StoryStage) => mount(ctx, options) },
  }
}
