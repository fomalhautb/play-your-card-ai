 ...
 * 那时烟尘已经扑开、追光正压在卡的右下角上、震屏还在最后一段位移里，三样东西同时看得见。
 * 手动时钟保证每次都停在同一帧，截图才比得动（6.6）。
 *
 * 为什么偏偏挑转角那一帧：追光沿的是卡的**圆角**矩形（见 fx/edgePath.ts），
 * 而这条路径最容易出错的地方就是四个转角——早先它走的是直角矩形，光点在角上会探到
 * 卡的圆角外面去。以前停在 250ms，那一刻光点正好在下边中点，走直角还是圆角一模一样，
 * 这条截图回归对转角一点保护都没有。200ms 落在右下角那段弧的中间，改坏了立刻看得出来。
 *
...
/**
 * 停在哪一帧。整段 0.8 秒，200ms 时烟尘、追光、震屏三样都在，
 * 而且追光正好压在右下角那段圆弧上（理由见文件头）。
