 ...
 *
 * 拍的是**播放中的一帧**：整段演出 0.8 秒，停在 200ms 那一帧上——
 * 那时烟尘已经扑开、亮弧的头部正压在卡的右下角上、震屏还在最后一段位移里，
 * 三样东西同时看得见。手动时钟保证每次都停在同一帧，截图才比得动（6.6）。
 *
 * 为什么偏偏挑转角那一帧：那圈环贴的是卡的**圆角**轮廓（见 fx/edgeRing.ts 的距离场），
 * 而最容易出错的地方就是四个转角——环在角上要么探到卡外、要么被削细一截。
 * 以前停在 250ms，那一刻亮弧正好在下边中点，那一段的环是直的，改坏了也看不出来。
 * 200ms 落在右下角那段圆弧的中间，圆角一有出入立刻看得见。
 *
...
/**
 * 停在哪一帧。整段 0.8 秒，200ms 时烟尘、亮环、震屏三样都在，
 * 而且亮弧的头部正好压在右下角那段圆弧上（理由见文件头）。
 * 手动时钟按 60fps 走整步，200 除得尽（12 步），换个数之前先算算落在哪一段。
 */
const SETTLE_MS = 200
...
/** 低档：3 团烟尘，没有落地亮环（TIER_CONFIG.low 的 edgeLight 是 false）。 */
export const TierLow = {
  name: '档位：低（3 团烟尘）',
  parameters: hitSpec('low'),
}

/** 中档：5 团烟尘加亮环，和旧版 DOM 那套的团数一致。 */
