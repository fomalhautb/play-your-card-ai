 ...
 *
 * **松手那一下这里就交班了**：判成打出就把牌原样留在拖拽层上、停在指针松开的地方，
 * 交给 `onPlay` 的实现（scenes/duel/input.ts）决定它接下来飞去哪儿。这里不自己接着演，
 * 是因为"飞去哪儿"要问版式和局面（落哪一格、技能牌是去中央亮相），而这个文件不认识它们。
 * 反过来，调用方**必须**接住每一条分支：没人接的那张会一直停在最顶上的拖拽层里，
 * 把底下的战场和手牌一起挡死，玩家看到的就是"牌拖到一半卡住了"。
 *
