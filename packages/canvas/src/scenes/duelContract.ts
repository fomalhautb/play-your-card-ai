...
export type DuelAnchorName =
  ...
  | 'battlefieldFoe'
  /**
   * 回合结算层底栏那颗「确认」。结算层没立着、或者还没演到「按钮淡入」那一拍时答不上来。
   *
   * 教程的步骤表**一句提示都没指着它**——那颗钮是结算层上唯一能点的东西，不用教。
   * 它在这张表里只为端到端：那条用例要替玩家点掉每一轮的确认，而它画在画布里，
   * DOM 里没有这颗按钮，位置又由结算层自己那套「设计尺寸排好再整块缩放」算出来。
   * 让场景回答，比在用例里把那套换算抄一遍稳（抄的那份迟早和这边对不上）。
   */
  | 'settleConfirm'
