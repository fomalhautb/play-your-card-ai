...
function spec(size: { width: number; height: number }, hover: number | null) {
  return {
    pixi: {
      ...
      settleMs: 900,
      /*
       * 单张截图给 150 秒，统一那档 45 秒对这三条不够（第 29、30 条第一次生成 linux 基线时，
       * 桌面档和人物高亮两条都是在这里超时的）。
       *
       * 这一页是目录页里最重的一条：光是建场景就要传九张 3344×1882 的整幅图，
       * 再为七个人和两层遮挡各烤一张 alpha 掩码——每张都是一次「渲染到纹理 + GPU 回读」。
       * CI 那台跑机上 WebGL 走的是 SwiftShader（纯 CPU 软件光栅），这些活比本机慢一个量级，
       * 而 Playwright 判「元素稳定」等的是合成器真出两帧，排在这批活后面就得一起等。
       * 本机（macOS）三条都在几百毫秒内拍完，所以这个数只是给慢机器留的上限，平时碰不到。
       *
       * 治本的办法是让这一页少吃点（比如给目录页专门备一份缩小的素材），
       * 但那要动素材流水线，和这一条的范围不搭；缩小 story 画布也不行——
       * 桌面档短边不能低于 768（见上面 DESKTOP 的注释），已经贴着下限了。
       */
