 ...
 * 状态矩阵：
 *   普通    三条，见下面
 *   悬停    不适用。悬停在 PlaqueButton 的条目里拍过，这里拍的是整页的版式
 *   按下    不适用，同上
 *   禁用    「双方就绪」那条里的「已准备」就是禁用档
 *   加载    「正在匹配…」那半档由 `phase: 'busy'` 摆出来，并到「空房」之外单开一条没有价值
 *
 * 画布取 720×520：比面板（560×400）大一圈，能一起拍到面板下面那个提示气泡。
...
function spec(view: RoomView) {
  return {
    pixi: {
      ...SIZE,
      // 提示气泡的淡入要 200 多毫秒，推到 600 毫秒它已经停住了（见 Bubble.ts）。
      settleMs: 600,
...
/** 双方都点过准备：「已准备」灰着，只等服务端开局。下面那句提示是掉线过一次的样子。 */
