 ...
 * 或者视口短边窄于断点，就走手机档。故意共用同一条判据——同一台设备在对局页和构筑页
...
export function pickDeckTier(width: number, height: number, coarsePointer = false): DeckLayoutTier {
  return pickTier(width, height, coarsePointer)
}

export function pickDeckLayout(width: number, height: number, coarsePointer = false): DeckLayout {
  return pickDeckTier(width, height, coarsePointer) === 'mobile'
