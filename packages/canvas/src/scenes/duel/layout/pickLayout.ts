 ...
 * 判进手机档。而手机档当时是**不缩放**的，顶栏加面板行加对手手牌条加手牌区在矮视口上
 * 会把战场挤没，于是牌拖上去也没有落点，看着就是「打不出牌」。
 *
 * 2026-09-17 起手机档矮视口会整块等比缩小（见 mobileLayout 的 stageScaleFor），
 * 上面那条「挤没」已经不会发生了；但这条判据仍然照旧看宽不看短边：手机档是给一列竖排、
 * 手指操作排的，鼠标电脑哪怕窗口矮，走**宽屏那一档**仍然更合适——整块缩小换来的是
 * 什么都变小，而桌面档在宽视口里本来就摆得开。缩放只是兜底，不是「两档可以互换」。
 *
