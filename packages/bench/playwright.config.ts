...
export default defineConfig({
   ...
   * 再重跑一轮验两遍完全一致。这台 M2 上最慢的一条（desktop/play10）单独跑要 4.4 分钟。
   *
   * 之所以不是 6 分钟：并行之后同一条用例会变慢。SwiftShader 是纯 CPU 的，
   * 两个 worker 一起跑就在抢同一批核（M2 八个逻辑核里只有 4 个是性能核），
   * 实测每条慢 1.4 到 1.9 倍，desktop/play10 从 4.4 分钟涨到 6.1 分钟，
   * 按 6 分钟给会当场超时。超时要按「并行时的最慢一条」给，不是按单独跑的。
   * 要再快只能少跑一轮或者降分辨率，那是改口径，另议。
   */
  timeout: 480_000,
