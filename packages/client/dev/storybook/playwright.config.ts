...
export default defineConfig({
  ...
  /*
   * 一条用例一片条目（分片的理由见 catalog.spec.ts），几片仍然**串着**跑。
   * 每个条目都要建一个 WebGL 上下文，而 Chromium 对同时存活的上下文数量有硬上限，
   * 抢起来会随机丢掉几个——并行只会把这道检查变得时好时坏。
   * CI 上的并行是**按 job 分**的：一格一台跑机，各跑一片（见 .github/workflows/ci.yml）。
   */
  workers: 1,
  /*
   * 单条用例 15 分钟。一条用例要把自己那一片的条目走一遍，每条都是
   * 「开页面 → 等画布就绪 → 截图比对」。整页一百多条本机（M 系列，软件渲染）实测约 3 分钟，
   * 分成两片各占一半；CI 那台两核跑机上重新生成一遍基线整页要 5 分多钟
