 ...
 * 分几片。
 *
 * 改这个数要**同时**改 `.github/workflows/ci.yml` 里 `catalog` 那个 job 和
 * `.github/workflows/catalog-baselines.yml` 的 matrix：两条工作流都按 `@shard<n>`
 * 这个标签筛用例，多出来的那一片会变成「一条用例都没匹配到」而不是失败，
 * 静悄悄地少拍一批条目。
 *
 * 从 2 改到 3 是迁移第 31 条改的：两片在跑机上已经到 8 分 43 秒 / 7 分 54 秒，
 * 而快档给每个 job 的口径是 10 分钟，这一条又要往目录页加二十多条 React 条目。
 */
const SHARDS = 3
 ...
 *
 * 不按 `Canvas/*` 和 `UI/*` 分，是因为两边条目数差得太远（现在是 149 比 40），
