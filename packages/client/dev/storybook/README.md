...
那套摆态连同样式一起删了。重做视觉时要么把 `data-state` 加回来，要么另议装伪类模拟插件。

拍得出来的那几档照拍：React 这半边现在只有**方块按钮**一种按钮（`ui` 的 `Button`），
它的「禁用」和「开着」（`pressed`，开关类按钮的当前值，不是瞬态）都真写在样式里，
各有一条条目。别的 React 组件仍然只有一条「普通」。
