/**
 * 组牌教学的数据与放行规则（搬自旧版 `legacy-client/test/tutorialDeck.test.ts`）。
 *
 * 界面那一半（挖洞高亮、点哪张卡）在 node 环境里跑不了，
...
import type { DeckStepId } from '../src/tutorial/deckSteps'
import {
  DECK_FIRST_STEP,
  DECK_STEPS,
  deckBlockTip,
...
describe('组牌教学的预填', () => {
  ...
  //「预填不会替玩家塞一张选不进牌组的牌」——真塞进去会被存档那边的规整悄悄丢掉，
  // 教学的 17/20 当场对不上。
  ...
  // 份数上限超了会被存档那边的规整悄悄丢掉，玩家点了卡却没进牌组，教学的计数就对不上了。
  it('拼出来的整副牌组没有一张超过每副的份数上限', () => {
    ...
    // 牌组套数上限归界面管，这里顺手确认一下那个常量还在（教程要覆盖的是固定 id 那一套）。
    expect(MAX_DECKS).toBeGreaterThan(0)
  })

  // 规格 §12 的顺序写死了：两张 AI 牌，再一张技能牌。
  it('待加的三张是「AI 牌、AI 牌、技能牌」', () => {
    const kinds = TUTORIAL_DECK_PICKS.map((cardId) => CARDS[cardId]?.kind)
...
describe('组牌教学的步骤与放行', () => {
  it('步骤按 next 串成一条链，最后一步收尾', () => {
    const visited: DeckStepId[] = []
    let id: DeckStepId | null = DECK_FIRST_STEP
    while (id !== null) {
  ...
  // 锁必须有话说：点了没反应的界面在玩家看来就是坏了。
  it('每一步被挡下时都有一句话，指名道姓说该点哪张', () => {
    expect(deckBlockTip(deckStep('DECK_INTRO'))).toBe('先看完这句提示')
    expect(deckBlockTip(deckStep('DECK_READY'))).toContain('确认牌组')
    const name = CARDS[TUTORIAL_DECK_PICKS[0]!]?.name ?? ''
    expect(deckBlockTip(deckStep('DECK_AI_1'))).toContain(name)
  })

  it('高亮目标要么是那两处固定锚点，要么是卡池里的一张卡', () => {
    expect(deckStep('DECK_INTRO').highlight).toEqual([{ kind: 'anchor', name: 'deckCounter' }])
    expect(deckStep('DECK_READY').highlight).toEqual([{ kind: 'anchor', name: 'deckConfirm' }])
    expect(deckStep('DECK_AI_1').highlight).toEqual([
      { kind: 'poolCard', cardId: TUTORIAL_DECK_PICKS[0] },
    ])
  })
})
