 ...
 * 这一版存在的意义只有一个——第 21、27 条要能「从头点进一局、打完、回来」，
 * 而那条路上必须有个起点。所以它只用 `ui` 的按钮 A，不摆任何美术资源。
 *
 * 两个单机入口都在这里建 driver 再跳 `/match`，理由见 app/MatchSession.tsx：
 * driver 得先交给挂在 Router 外面的那个 Provider，跳转才带得走。
 * 「联机对战」不建 driver——联机的 driver 要有房间码才建得出来，那是房间页的事。
 */
  ...
  return (
    <main className="home">
      ...
      <p className="home__note">
        正式版施工中。真首页是迁移第 30 条的事，这一版只留三个进对局的入口。
      </p>
      <div className="home__actions">
        ...
        <Button onClick={() => navigate('/room')}>联机对战</Button>
      </div>
      {/*
