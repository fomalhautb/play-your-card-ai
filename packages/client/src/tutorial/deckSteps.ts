 ...
 *
 * 从旧版 `legacy-client/src/tutorial/deckSteps.ts` 搬过来，改了两处：
 * 数据从 `@ai-duel/content` 查（旧版那时卡池还在 core 里），
 * 以及**高亮不再是 CSS 选择器**——构筑页整页画在画布上，DOM 里一个卡格都没有，
 * 目标改成画布那边答得上来的语义锚点（`DeckScene.anchorRect`，类型就是它的 `DeckAnchor`）。
 *
 * 和对战那张步骤表（steps.ts）一个路子：纯数据 + 纯函数，不认屏幕坐标，
...
import type { DeckAnchor } from '@ai-duel/canvas'
import { CARD_POOL, CARDS, DECK_SIZE } from '@ai-duel/content'
 ...
 *
 * 挑的都是教学对战里会用到的牌——第 1 轮派出去的 GPT-3.5、教学局起手里那张豆包、
 * 第 2 轮打出去的「复读机」。组牌排在对战之前（见 screens/TutorialScreen.tsx 的 phase 顺序），
 * 所以这一层关系是**反过来**的：玩家先在这里认识这三张，等下在对战里又见到它们。
 */
 ...
 * 全部通过。取自 `CARD_POOL` 还顺带保证了预填里不会混进两类灰卡——
...
export interface DeckStep {
   ...
   *（那一档由 `DeckScene.setTutorial` 统一挡，见 canvas 的 deckContract.ts）。
   */
...
export function deckBlockTip(step: DeckStep): string {
  ...
  const name = CARDS[step.allowedCardId]?.name ?? step.allowedCardId
  return `教学阶段：先加入高亮的那张「${name}」`
}
