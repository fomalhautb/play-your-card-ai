 ...
 *
 * 从旧版 `legacy-client/src/tutorial/content.ts` 原样搬过来，只换了取数据的地方
 *（题库和卡表现在从 `@ai-duel/content` 查，旧版那时它们还在 core 里）。
 *
 * 教学局跑的是真引擎（《正式版架构》5.3），题目和回答也和正式对局同源：
 * 题目从 `QUESTION_POOL` 里挑三道，回答走 `scriptedAnswers`，
 * 也就是那份离线跑出来的真实模型回答表（见 content 的 script.ts）。
 * 所以「可预测」不是靠手写答案，而是靠把随机整个关掉（`noShuffle` 按数组顺序发牌、
 * `firstPlayer` 指定先手、`questions` 直接给三道题），再**按那张表挑题和挑牌**：
 * 每一轮玩家场上那张答得对、对手脚本派的那张答得错。
 * 这份文件里的每个数字和每次挑选都被 test/tutorialContent.test.ts 守着，改之前先看那几条断言。
 *
 * 术语：数组里写的是**抽牌顺序**，真正的牌组要把它倒过来——引擎抽牌是从数组末尾 pop 的
 *（见 core 的 `GameSetup.noShuffle`）。所以下面统一写「抽牌顺序」，最后一步才 reverse。
 */

import { CARDS, QUESTION_POOL } from '@ai-duel/content'
 ...
 * 测试里的「每一轮的对错都是表里查出来的」和三轮比分对账会当场报错。
 */
...
export const TUTORIAL_QUESTIONS: Question[] = TUTORIAL_QUESTION_IDS.map((id) => {
  ...
  // 找不到说明 questions.json 改了 id。教学局照样开得起来，但会开成一局
...
export const TUTORIAL_CARDS = {
   ...
   *（见 content 的 `UNAVAILABLE_AI_CARD_IDS`），玩家学完回到牌组页会发现刚用过的那张是灰的、
   * 自己拼不出这副牌。宁可让第 2 轮少一个可选动作，也不留这处对不上。
   *
   * 换成卡池里最便宜的 2 费 AI 在算术上是可行的——第 2 轮上限 6 点，
 ...
 *
 * 直接查 `CARDS` 而不是 core 的 `getCard`：后者要一份 `Catalog`，
 * 而这个模块只是一张数据表，手上没有对局状态可拿。
 */
export function tutorialCardCost(cardId: CardId): number {
  const card = CARDS[cardId]
  if (card === undefined) throw new Error(`教学局用到了卡表里没有的牌：${cardId}`)
  return card.tokenCost
}
