 ...
 *
 * 从旧版 `legacy-client/src/tutorial/heroSteps.ts` 搬过来，只把高亮从 CSS 选择器
 * 换成画布那边答得上来的语义锚点（`HeroScene.anchorRect` 的 `HeroAnchor`）。
 *
 * 只有两步，所以没做成 deckSteps.ts 那样的通用表：
 * 「点她」和「确认」各一句话，推进条件就是技能详情开没开。
 */

import type { HeroAnchor } from '@ai-duel/canvas'
 ...
 * 这一步等于把等下那局要用的英雄先定下来；而且她的 Debug 是被动，
 ...
 *
 * 玩家在详情里点「返回」会退回 `HERO_PICK`，不会卡死（详情开关由界面回调驱动）。
 */
...
export interface HeroStep {
  ...
  instruction: string
  /** 要挖洞高亮的那一处。 */
  highlight: HeroAnchor
  /**
   * 压暗无关区域。
   *
   * 确认那一步是 false：技能详情自己已经铺了一层压暗的遮罩，
...
export const HERO_STEPS: Record<HeroStepId, HeroStep> = {
  HERO_PICK: {
    ...
    highlight: { kind: 'heroCard', hero: TUTORIAL_HERO },
    dim: true,
  },
  HERO_CONFIRM: {
    id: 'HERO_CONFIRM',
    instruction: '就选她。',
    highlight: { kind: 'heroConfirm' },
    dim: false,
  },
}
