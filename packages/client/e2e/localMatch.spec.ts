...
test('从首页开一局测试对局，拖牌出牌，一路打到结算页', async ({ page }) => {
   ...
   * 第一步：真的拖一张牌出去。
   *
   * 重试是必要的，不是掩盖不稳定：开局要先演抛硬币（3.54 秒）再发牌，这期间手牌是锁着的
   *（编排层的 dealing 锁），拖了也不会有反应。用「手牌少了一张」当成功判据，
   * 而不是等一个固定的时长——那个时长会跟着演出时长一起漂。
   */
  ...
  let played = false
  /*
   * 一张张试过去，不是只试中间那张：手上五张里可能好几张现在打不出去——
   * 要选目标的技能牌（拖出去只会立起选目标层）、Token 不够的贵牌（引擎直接拒）。
   * 两圈够把五张都试一遍还有余量。
   *
   * 每次之间要等满 2.8 秒：出牌那一下编排层就把演出锁上了，指令被拒时那把锁靠
   * PLAY_LOCK_FALLBACK_MS（2.5 秒）兜底放开，没等满的话下一次拖拽会被锁挡掉，
   * 白白浪费一张牌的机会。开局那段（抛硬币 3.54 秒 + 发牌）也一样是锁着的。
   */
