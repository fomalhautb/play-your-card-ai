 ...
 * 正式版简化第 4 步剥成素方块之后那批图删了、闸门也没了，最后要图的那一排展示卡
 * 也删了，所以这里等的就是画布本身——判据见下面的 `waitForScene`。
 */
 ...
 * 一个 await（建渲染器；对局那一页还要装卡面图集）才建得起来。这中间点上去，
 ...
 *
 * 对局那一页的图集要现下，第一次进站要几秒，所以给足 60 秒（这个函数两页共用）。
 */
