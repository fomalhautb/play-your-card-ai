...
import { Client, setupRoom, signForgedToken, signToken } from './helpers'
...
describe('房间握手', () => {
  it('带对的 token 能进，101 只回显 ai-duel，welcome 带座位', async () => {
    ...
    const alice = await Client.connect('1000', await signToken('alice'))
    ...
    expect(welcome.userId).toBe('alice')
  ...
  it('座位号由服务端按建房时的名单分，客户端说了不算', async () => {
    ...
    const bob = await Client.connect('1001', await signToken('bob'))
    bob.send(HELLO)
  ...
  it('伪造签名的 token 进不去', async () => {
    ...
    const forger = await Client.connect('1004', await signForgedToken('alice'))
  ...
  it('过期的 token 进不去', async () => {
    ...
    const late = await Client.connect('1005', await signToken('alice', '-10s'))
  ...
  it('第三个人进不了满房', async () => {
    ...
    const carol = await Client.connect('1006', await signToken('carol'))
  ...
  it('房间还没建就来连的，回 room-not-found', async () => {
    const early = await Client.connect('1007', await signToken('alice'))
  ...
  it('协议版本对不上：先说清楚再关，不是握手那一步失败', async () => {
    ...
    const old = await Client.connect('1008', await signToken('alice'))
  ...
  it('同一个账号再连一条，旧的被顶掉，座位不变', async () => {
    ...
    const first = await Client.connect('1009', await signToken('alice'))
    first.send(HELLO)
    await first.expect('session:welcome')
    // welcome 之后紧跟一条对手状态，先取掉，下面等的才是顶号那条。
    await first.expect('room:peer')

    const second = await Client.connect('1009', await signToken('alice'))
  ...
  it('没打招呼就发别的消息，一律不认', async () => {
    ...
    const alice = await Client.connect('1010', await signToken('alice'))
  ...
  it('乱码和不是发给房间的消息都回 malformed', async () => {
    ...
    const alice = await Client.connect('1011', await signToken('alice'))
    alice.send(HELLO)
