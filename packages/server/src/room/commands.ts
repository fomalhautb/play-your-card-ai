...
import { scheduleAnswers } from './autopilot'
import { dispatchEvents } from './dispatch'
import { closeRoom } from './lifecycle'
import { sendRoomError } from './session'
 ...
 * 执行一条指令，把结果发出去。
 *
 * 服务端自己发的 `SUBMIT_ANSWERS` 也走这里（见 MatchRoom 的 `alarm`），
...
export async function runCommand(
  ...
  // 排答题 alarm 排在下发之前：定时器先落盘，客户端才可能看到「题目揭晓了」。
  // 反过来的话，客户端已经在演答题动画、服务端那个定时器却还没写进去。
  await scheduleAnswers(room, result.events)
  ...
  if (result.events.some((event) => event.type === 'GAME_OVER')) {
    await closeRoom(room, 'match-over', '这一局打完了')
  }
...
export async function handlePlayerCommand(
  ...
  await runCommand(room, command, ws)
}
