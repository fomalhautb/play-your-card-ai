 ...
 * - 新的（`/match/:code`）是权威房间对象，规则在服务端跑，实现在 src/room/。
 *
 * 两套各有各的 Durable Object 绑定（`ROOM` / `MATCH_ROOM`）和各自的类，互不相干。
...
export default {
  async fetch(request, env) {
    ...
    const match = MATCH_PATH.exec(new URL(request.url).pathname)
    if (match) {
      // 这个地址只接 WebSocket，不像旧的 /room/:code 还兼着前端路由，
      // 所以非升级请求直接回 426 而不是回 index.html。
      if (request.headers.get('Upgrade')?.toLowerCase() !== 'websocket') {
        return new Response('这个地址只接 WebSocket 升级请求', { status: 426 })
      }
