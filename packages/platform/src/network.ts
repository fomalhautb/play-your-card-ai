 ...
 *（旧代码把两件事挤在黑客松版的 src/net/socket.ts 一个文件里，重写时按这条缝切开）。
 *
...
export interface SocketOptions {
  ...
  url(): string
  /**
   * 每次（重）连之前调一次，返回这一次要提的 WebSocket 子协议名。
   *
   * 也写成函数，但理由和 url 不同：升级请求里唯一能塞凭据的地方就是这个头
   *（浏览器的 WebSocket 构造函数设不了别的 header），而凭据是短时效的，
   * 挂了半小时之后重连必须现换一张。返回 Promise 是因为换凭据要走一次请求。
   *
   * 这一层不认识凭据长什么样，只负责「每次连接现取一遍」。
   */
