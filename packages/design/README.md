...
- **动效时长**：手牌重排 0.4s、卡牌放大进出场 0.55 / 0.6s、卡片加载周期 1.75s、
  ...
  气泡的淡入淡出和停留，以及四条**环境动画**：结算层等待点跳动、结算层比分脉冲、
  展示层停留时的浮动、选目标层压暗的淡入淡出。
  收环境动画这四条的判据和别的时长不同：它们不是某一段演出的长度（那些在 canvas 的
  `director/timings.ts` 里，一条一条对着旧代码抄），而是「屏幕上有东西在轻轻动」的统一节奏，
  一处快一处慢会让画面看着不是一套。真正只服务一段演出的时长（拖拽、翻面）仍留在组件里。
