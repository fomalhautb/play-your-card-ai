import type { AiCard, CardId } from '@ai-duel/core'

/**
 * 十八张具名 AI 牌。
 *
 * 每张都有一张专属原画，文件名就是这里的 id（映射规则见 test/assets.test.ts），
 * 所以改 id 等于换图，必须两边一起改。
 *
 ...
 * 规则那边不读这份链：引擎的 upgradeTargetOf / downgradeTargetOf 直接照 `evolvesTo` 现算
 *（见 core 的 catalog.ts），所以两处看的是同一份字段，不会各自漂移。
 * 这份链是给"要一整条代际关系"的人用的：牌组页画进化链、测试逐级核对升降级。
 *
 * 升降级技能就是**把场上那个单位的 `cardId` 换成相邻的一代**，不另加任何数值修正：
 * 答题表现本来就写在 script.ts 那张「题目 × 卡牌」的静态表里，换了卡自然就换了一整套答题结果，
 * 引擎不必再理解"强了多少"。费用也不用重算——技能是免费换卡，不退不补（见 core 的 useHeroSkill）。
 *
