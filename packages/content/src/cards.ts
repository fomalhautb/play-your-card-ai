...
/*
 * 按 id 取卡牌定义的 getCard 不在这里：它是规则的一部分，读的是一局开始时冻结进
 * GameState 的那份目录（core 的 catalog.ts），而不是这张随版本变动的总表。
 */
 ...
 * 客户端把它们播成最初的三套牌组存档（见黑客松版的 deckStore）。
 *
 ...
 * 而不是等玩家开局才发现。
 *
 * 一局最多摸 5（起手）+ 8（第 2~5 轮各 2 张，见 core 的 ROUND_DRAW_SIZE）= 13 张，
 * 20 张管够，三副都不会抽空。
 *
 ...
 *
 * 「模型蒸馏」2 张是这副牌唯一的起手解：第 1 轮上限只有 5 点（见 core 的
